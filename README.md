 ...
 * **Download the Repository:**  **Download ZIP** (or clone the repository). Extract the files(the UAV_Workspace_engine $ UAV Frontend index.html $ bridge.py) to a folder.
 ...
 * **g++ UAV_workspace_Engine.cpp -o UAV_workspace_Engine.exe**
  ...
  * # ## video attached for same
   https://github.com/user-attachments/assets/a9cca433-2982-42d6-9fa1-773060edb187
  * # ##Demo video of Web
...
* **Frontend UI/Layout (UAV frontend.html + CSS)** — The interactive dashboard layout, card-based slider system (Card A: Sizing Constraints, Card B: Environment & Balance), the live canvas schematic (top-down wing view, V-tail side profile, dimension-line annotations), all the Outputs realtime aerodynamic calculations for main wing/tail surface sizing flight performance metrics (Reynolds number stall/cruise speeds), and fluid forces (lift and drag parameters) and the aerodynamic guide modal were designed and built by me.
* **AI ASSISTANCE USED IN Building the Bridge.py and function runaerodynamicpipeline()(in index.html): were used to help scaffold the glue layer connecting the two — the Python bridge server and the JS data-fetching pipeline**
